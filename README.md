<sub>onto</sub>
### Ravi Pratap Singh
#### [Founder @ Open Dev Society](https://github.com/Open-Dev-Society) | [Founder @ Onto](https://buildonto.dev)

> **Status:** Building the infrastructure layer for the AI-native web.  
> **Philosophy:** Knowledge should be free. Software should be agent-native.

| Project | Status | Impact |
| :--- | :--- | :--- |
| **[Onto](https://buildonto.dev)** | `v1.0-alpha` | Eliminating the "React Tax" for AI Agents. |
| **[OpenStock](https://github.com/Open-Dev-Society/OpenStock)** | `Production` | 19,200+ Stars | 10,500+ Active Users. |
| **[KineUI](https://kine-ui.vercel.app/)** | `Beta` | Spatial gesture interactions @ <16ms latency. |

#### ✦ Impact
- **Total Stars:** 13,300+ across Open Dev Society repositories.
- **Total Users:** 10,500+ monthly active users on open-source production tools.
- **Recognition:** GitHub Trending Repo (Globally), featured on HelloGitHub.
- **Validation:** YC Startup School attendee, Selected for Immersion by Google for Startup powered by Antler.

#### ✦ Stack
[![My Skills](https://skillicons.dev/icons?i=ts,next,supabase,vercel,vite,threejs,npm,docker,cloudflare,c,bash,ubuntu)](https://skillicons.dev)

#### ✦ The Manifesto
I believe that being a student from a middle-class family shouldn't limit your vision. I build because I have to, using free-tier tools and a "scavenge for parts" mindset to ship production-grade infrastructure.

- **Founder @ Open Dev Society:** 1,000+ members strong.
- **AIO Advocate:** Measuring the web by its readability for the next species of users—AI Agents.

#### ✦ Connect
- **Website:** [buildonto.dev](https://buildonto.dev)
- **LinkedIn:** [in/ravixalgorithm](https://www.linkedin.com/in/ravixalgorithm)
- **Portfolio:** [ravixalgorithm.tech](https://ravixalgorithm.tech)
